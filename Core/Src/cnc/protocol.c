...
static void protocol_exec_rt_suspend(){
  ...
  uint8_t restore_condition;
    ...
    #endif
  #else
    if (block == NULL) { restore_condition = (gc_state.modal.spindle | gc_state.modal.coolant); } else { restore_condition = block->condition; }
  #endif
      ...
      if (sys.state & (STATE_SAFETY_DOOR | STATE_LOCKED)) {
        ...
        if (bit_isfalse(sys.suspend,SUSPEND_RETRACT_COMPLETE)) {
          ...
          sys.spindle_stop_ovr = SPINDLE_STOP_OVR_DISABLED;
            ...
            if ((bit_istrue(settings.flags,BITFLAG_HOMING_ENABLE)) && (parking_target[PARKING_AXIS] < PARKING_TARGET) &&
                 bit_isfalse(settings.flags,BITFLAG_LASER_MODE) && (sys.override_ctrl == OVERRIDE_PARKING_MOTION)) {
            #else
          ...
          if (sys.suspend & SUSPEND_INITIATE_RESTORE) {
            ...
            if (gc_state.modal.coolant != COOLANT_DISABLE) {
              ...
              if (bit_isfalse(sys.suspend,SUSPEND_RESTART_RETRACT)) {
                ...
                //udelay_sec(SAFETY_DOOR_COOLANT_DELAY, DELAY_MODE_SYS_SUSPEND);
              }
            }
